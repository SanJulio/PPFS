...
- `tests/` — pytest suite (467 tests, all passing); `conftest.py` has SQLite schema matching production, including all August 2026 columns
  ...
  - `tests/test_goals.py`, `tests/test_home_goals.py`, `tests/test_goal_pace_projection.py` — savings & debt repayment goal tracking (39 + 15 + 30 tests)
...
- **Savings & Debt Repayment Goal Tracking**: full design as documented above — new `goals`/`goal_contributions` tables, three progress-calculation modes (linked savings/linked debt/standalone), deterministic (non-AI) pace suggestion cross-checked against Safe to Spend, manual + automatic completion, a deliberate judgement call on locked-linked-account handling (flagged, not excluded), a Home page entry point, and — added in a follow-up pass after a user-caught gap — a genuine projected-completion-date-from-real-recent-pace feature (the inverse of the pace suggestion), with a years-away cap so an extremely slow pace shows an honest rough figure instead of a fabricated-looking precise date decades out. Explicitly ships without the streak/engagement layer, which is a separate not-yet-designed follow-up. 84 new tests across three files; also fixed a pre-existing gap in `conftest.py`'s test cleanup that the first round of new tests surfaced.
- Test suite grew from 228 → 467 tests across this work, all passing.
