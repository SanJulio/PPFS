...
**Tests**: `tests/test_goal_fallback_pace.py` (23 tests) — the fallback appearing in place of the blank insufficient-data message, its distinct labelling/styling, switch-over to real pace for both standalone and linked goals, the even split across 2+ active goals (verified via real `/manage` requests, not a direct function call, per the Flask-Login note above, using large goal targets so the new cap doesn't mask the split-evenly arithmetic), a real-pace goal correctly excluded from the fallback denominator, a completed goal excluded too, and the zero/negative-Safe-to-Spend case producing no fabricated pace. Plus 5 tests for the August 2026 fix: a direct regression repro of the original bug's account (confirms the figure is now capped near the goal's own target instead of far exceeding it), a direct invariant check that the capped pace never implies completing a goal in under ~30 days regardless of how large the underlying balance/income is, confirmation the cap does NOT suppress a genuinely large surplus against a genuinely large goal, a mocked check that `_safe_to_spend_daily_rate()` no longer depends on "today" (fixed cycle length denominator, not days-remaining), and a zero-remaining-amount edge case for the cap helper itself. One pre-existing test in `test_goal_pace_projection.py` (`test_recalculates_as_linked_balance_changes`) had its expectations updated — its account's positive balance now correctly triggers the fallback estimate before real transaction history exists, instead of the old blank message.
