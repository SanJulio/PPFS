...
## Performance: per-request connection sharing (August 2026)

**Investigated first, not assumed**: a report that every tab felt slow (Home worst) was profiled directly against staging Postgres (real network round-trips, not local SQLite) before any fix was proposed. Wrapped `psycopg2.connect`/`cursor.execute` to count and time every physical connection and query per route. Finding: **query execution and computation were never the problem** — Home's Safe to Spend/forecast maths and its 34 queries totalled ~1.6s, but the route took 20–23s wall time. The other ~20s was connection *setup* time, invisible to query-level profiling.

**Root cause**: `database.get_db()` opened a **brand-new physical connection on every call**, with no reuse within a request — confirmed 139 call sites across app.py, several of them inside `calculate_financial_overview()` alone (separate connections for accounts, future events, savings rules, income). A live-measured Home request opened **28 separate connections**; Forecast/Transactions/Manage opened 5–10. Two of those connections happened on *every* route regardless of page weight: `PostgresSessionInterface._get_db()` (`open_session()`/`save_session()`) calls `psycopg2.connect()` directly, bypassing `database.get_db()` entirely, on every single request. Confirmed via `grep` that zero of the 139 `get_db()` call sites used `try/finally` — meaning a connection opened before an unhandled exception was already leaking today, independent of this fix.

**Fix — one shared connection per request via `flask.g`, not a pool.** `get_db()` now opens a connection lazily on first use within a request, stores it on `g`, and returns the same one for every subsequent call in that request. `close_db()` (registered as `app.teardown_appcontext(close_db)` in app.py) closes it unconditionally when the request ends — success or exception. `release_db()` is now a no-op, kept only so its 139 existing call sites needed zero changes.

**Why this doesn't reintroduce the pooling-caused exhaustion crashes** (see "Sessions" above): a pool keeps a fixed set of connections alive *between* requests — a persistent structure that can drift out of sync with Postgres's connection limit if sizing or return-to-pool logic is off, which is almost certainly what caused the earlier crashes. This pattern keeps nothing alive between requests at all: a connection is opened only when a request actually needs one and is torn down the moment that same request ends, via Flask's own `g` + `teardown_appcontext` (the framework's documented pattern for exactly this, and thread-safe via Werkzeug's context-local — verified safe even under the local dev server's `threaded=True`). The maximum number of connections open at any instant is bounded by concurrent in-flight requests, same as before — the only change is collapsing many sequential opens-and-closes *within* one request into one held for that request's duration. As a side effect this also closes the pre-existing exception-leak gap noted above, since `teardown_appcontext` runs unconditionally where the old per-call-site pattern didn't.

**Session interface deliberately left untouched.** Checked Flask's actual dispatch order: the app context (where `g` lives) is pushed before `open_session()` runs and torn down after `save_session()` runs, so those two connections *could* technically share the same `g`-based connection. Decided against it: if a request's business logic hits a DB error mid-transaction, Postgres puts that connection into an aborted-transaction state where every subsequent statement fails until a rollback — sharing it would mean a business-logic failure could also silently break session persistence (login state, CSRF token, flash messages) at exactly the moment it matters most. This leaves 2 connections/request as an accepted residual cost — the user's own call, made explicitly aware of the tradeoff, not a technical limitation. Revisit only if profiling later shows it's worth the added complexity of defensive aborted-transaction handling.

**Measured result** (staging, same account/data as the investigation): every route dropped to exactly 3 connections (1 shared business connection + the 2 session ones). Home: 28→3 connections, 20–23s→3.6–4.5s wall time (~83% faster). Manage: 10→3, 8.1–8.6s→2.2–3.9s. Transactions: 7→3, 6.4–7.2s→1.7–2.1s. Forecast: 5–8→3, 3.5–5.5s→1.7–2.9s. Remaining time is now almost entirely those 3 connections' own setup cost over the public-internet path to staging — production's internal Render network path (see the TLS section above) should make this meaningfully faster still, though that wasn't separately measured.

**Compatibility fix required**: switching `get_db()` to depend on `flask.g` surfaced 68 test failures (`RuntimeError: Working outside of application context`) — tests calling DB-touching `app.py` helpers directly without a Flask context, since the old `get_db()` had no Flask dependency at all. 67 of the 68 shared the `db_conn` fixture, so wrapping it in `with app.app_context():` (`tests/conftest.py`) fixed them in one change; the one outlier test had no fixtures at all and got its own small fix. `init_db()` (called once at startup, outside any request) was already wrapped in `app.app_context()` in app.py, so needed no change.

**Also confirmed during the investigation, not yet acted on**: `calculate_financial_overview()` has no caching at all (fully recomputed every Home load — real, but dwarfed by the connection issue, since its own query time was only ~1.6s of Home's 20s+). The 5-minute `forecast_cache` does work (Forecast's query count dropped from 9 cold to 3 warm), but its benefit was invisible next to the connection overhead. Every response — including static assets like the logo/icons — carries `Cache-Control: no-store` from the app-wide `add_no_cache_headers()` hook, so browsers can't cache them client-side across tab navigation; this is a deliberate blanket security policy (stops logged-out users seeing cached authenticated pages via back button) with a real secondary cost for static-asset re-fetching, not touched by this fix. One minor N+1-shaped query repeat was found on Home (`SELECT COALESCE(SUM(amount)...)` ×4) but is negligible next to the connection-count issue.
...
- **DB connections**: `get_db()` opens (lazily, once) and shares one connection per request via `flask.g`, closed unconditionally by `close_db()` at `teardown_appcontext` (registered in app.py right after `app = Flask(__name__)`). `release_db()` is now a no-op, kept only so its ~139 existing call sites didn't need touching. **Not a connection pool** — nothing persists between requests, so this doesn't reintroduce the exhaustion crashes pooling caused; see "Performance: per-request connection sharing" below for the full before/after and the reasoning. Import pattern: `from database import get_db, USE_POSTGRES` inside route functions.
...
### September 2026 — PWA manifest: scope/lang/dir + wide screenshots
- Added `scope: "/"`, `lang: "en-GB"`, `dir: "ltr"` — plain top-level fields, no code changes. `related_applications` deliberately parked (needs a real Play package URL to point to first); everything else PWABuilder flagged (`display_override`, `share_target`, `file_handlers`, push/background-sync, etc.) is a real feature decision or genuinely not applicable, not manifest cleanup — left alone on purpose.
- Added three `form_factor: "wide"` screenshots (1280×1100) alongside the existing `"narrow"` ones, same Home/Forecast/Goals pages, same corrected seed data (real account on the income row, payday timed relative to "today", real Savings transaction history for a believable goal pace) from the narrow-screenshot pass — reused rather than re-derived, since it was already proven correct. The app has no dedicated wide/tablet layout (deliberately portrait-only), so a wide capture is just the same responsive content with more surrounding canvas — an accepted pattern for a mobile-first app's wide store screenshots. First pass at 1280×800 cut off the Forecast chart and the Goals card's lower half; widened the *height* to 1100 (not width) to fit full content, which also revealed the Home page's Goals summary card that wasn't visible in the narrower/shorter first attempt.

### September 2026 — PWA manifest additions (id, categories, orientation, screenshots)
- Addressed a few items from PWABuilder's flagged list: `id: "/"` (a stable identifier independent of `start_url`, so the app isn't misidentified if the entry URL ever changes), `categories: ["finance"]`, `orientation: "portrait"` (phone-only, no landscape use case). All three are plain top-level `manifest.json` fields, no code changes needed.
- Added a real `screenshots` array (Home, Forecast, Goals) — captured via a disposable local Playwright harness (temp SQLite DB seeded with realistic accounts/bills/income/goals, `SecureCookieSessionInterface` swapped in since the production `PostgresSessionInterface` no-ops without a real `DATABASE_URL`, matching the established local-verification pattern) rather than reusing an old marketing screenshot, so what's shown is genuinely current UI. Captured at a 412×915 mobile viewport (`form_factor: "narrow"`, matching the portrait-only orientation above) with the Home page's privacy-blur toggled off first (`#toggleAmountsBtn`) so real figures are visible. Saved to `static/screenshots/` and referenced from `manifest.json`; useful for the Google Play listing too, not just the manifest.
- **Two of the three needed a retake after review — both traced to real seed-data bugs, not the app.** (1) Forecast initially showed the balance chart draining to nothing with no recovery: the seeded income row used `account=""` (the convention most other test fixtures use, since the specific account usually doesn't matter for what they're testing) — but `forecast()`/`api_snapshot()` check `inc["account"] in accounts`, so an empty account silently excludes the income from the simulation entirely. Fixed by pointing it at a real account name, and computed the payday as `today + 9 days` (rather than a fixed day-of-month) so it reliably falls inside the Forecast chart's default ~11-day view — otherwise a distant payday leaves the recovery off-screen even though the underlying 90-day numbers are healthy. (Tried re-centring the chart via `?date=`, the existing jump-to-day mechanism, first - rejected because it also opens the per-day detail panel, cluttering the screenshot.) (2) Goals showed the Emergency Fund goal completing implausibly soon: with no real transactions on the linked Savings account, `_compute_goal_recent_pace()` had nothing to reconstruct a real pace from, so it fell through to the Safe-to-Spend fallback estimate - which, against a healthy seeded income minus modest bills, implied the *entire* monthly surplus was going toward this one goal alone. Fixed by seeding three realistic £350 monthly top-up transactions on Savings, giving the goal a genuine tracked pace (card now reads "At current pace", not the italicised "Estimated" fallback label) that lands close to - just ahead of - its target date instead of a year-plus early.
- **Investigated PWABuilder's "Offline capability check failed due to response" flag, reported back before touching anything (as asked) — confirmed expected, not a bug.** The service worker's `fetch` handler deliberately never intercepts page navigations (only same-origin `/static/*` GETs — see "PWA service worker" above), so a genuinely offline load of `/` hits a real network failure rather than anything the SW could serve, which is exactly what the check is detecting. This is the direct, working consequence of the already-documented "cache-first for static assets only, no offline transaction data" scope — showing stale account balances offline would be actively misleading for a financial app. Flagged a narrower, safer middle ground (a static offline fallback page with no real data, just "you're offline") as a real option if ever wanted, but left it unbuilt pending a decision.

### August 2026 — Performance: per-request connection sharing (Stage 1)
- Investigated a report that every tab felt slow (Home worst) by profiling actual connection counts and query timing against staging Postgres, rather than guessing. Found the real cause wasn't queries or computation (Home's own query time was ~1.6s) but `database.get_db()` opening a brand-new physical connection on every one of its 139 call sites — a live Home request opened 28 separate connections, each costing hundreds of milliseconds of setup. Scoped a fix before building it, given CLAUDE.md's own documented history of a prior connection-*pooling* attempt causing exhaustion crashes: confirmed explicitly why a per-request `flask.g`-shared connection (opened once, closed unconditionally at `teardown_appcontext`) is structurally different from a pool — nothing persists between requests, so there's no standing structure to leak or mis-size. Deliberately left the session interface's 2 separate per-request connections (`PostgresSessionInterface`) untouched — technically could share the same connection, but isolating session persistence from business-logic transaction failures was judged worth the residual cost, a decision made explicitly rather than defaulted into. Measured result: every route dropped to exactly 3 connections; Home went from 28 connections/20–23s to 3 connections/3.6–4.5s. Full design and numbers in "Performance: per-request connection sharing" above. Surfaced 68 test failures (calling DB-touching helpers directly without a Flask context, since the old `get_db()` had no Flask dependency) — 67 fixed by one `db_conn` fixture change in `tests/conftest.py`, one outlier fixed individually.
- Stage 2 (unifying the session interface's 2 connections into the same shared one) was scoped but deliberately not built — the user's call, given the remaining gain (3→1 connections) is small against Stage 1's already-massive win and the correctness risk isn't worth it right now.
- No new tests added (this was a shared-infrastructure fix proven via live connection-count profiling, not new app behaviour) — existing 702-test suite passes unchanged.
